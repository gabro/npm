...
var warnInvalidHttps = require("../utils/warn-deprecated.js")("dependencies")
...
function addRemoteGit (uri, _cb) {
  ...
  if (uri.match(/^git\+https?:\/\/.*:.*/)) {
    var correctedHttp = uri.replace(/^(git\+https?:\/\/[^:]*):(.*)/, '$1/$2')
    var correctedSsh = uri.replace(/^git\+https?/, 'git+ssh')
    warnInvalidHttps([
      "The dependency specifier " + uri + " is using an invalid URI",
      "syntax, which has been deprecated.",
      "Use '" + correctedHttp + "'",
      "or '" + correctedSsh + "' instead"
    ], uri)
  }
